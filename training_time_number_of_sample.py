...
for i in sample:
    ...
    for k in range(num_iter):
        t_FEM_Data = FEM_data(no_element=63, num_samples_train=i)
        train_time_FEM_Data.append(t_FEM_Data)

    for l in range(num_iter):
        t_FEM_NN = FEM_NN(no_element=63, num_samples_train=i)
        train_time_FEM_NN.append(t_FEM_NN)
    
    for m in range(num_iter):
        t_PINN = PINN(no_element=64, num_samples_train=i)
...
#plt.ylim((None,25))
plt.savefig('No of sample time.png')