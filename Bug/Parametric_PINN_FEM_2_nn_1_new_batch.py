...
optimizer = torch.optim.LBFGS(ansatz.parameters(),lr=1.0,
        max_iter=20,
        max_eval=25,
        tolerance_grad=1e-9,
...
F = torch.cat(F, dim=0)
print(F)
u_real = linalg.solve(K_main,F)
u_real  = torch.from_numpy(u_real).float()
print("np sol:\n", u_real)
...
x_in = Variable(torch.from_numpy(np.linspace(0, 1, n_node ).reshape(-1, 1)).float(), requires_grad=True)
E1 = torch.ones(num_points_pde,1)*youngs_modulus


def normalize_input_pre(x,E):
    x_min = min(x)
    X_max = max(x)
    x_nor = (x - x_min)/(X_max-x_min)
    print("X_nor:",x_nor)
    range_E = max_youngs_modulus-min_youngs_modulus
    E_nor = (E - min_youngs_modulus)/range_E
    print("E_nor:",E_nor)
    return x_nor,E_nor

x_in,E1 = normalize_input_pre(x_in,E1)
print('input',)
#inputs1 = {'coords': x, 'youngs_modulus': E1 }
inputs1=torch.concat((x_in, E1), dim=1)
print(inputs1)
u = ansatz(inputs1)
print('predicted1:',u)
